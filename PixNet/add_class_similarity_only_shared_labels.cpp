...
void add_class_similarity_only_shared_labels(float **val, int **ind, int N1, int knn, int **sup_loc, int **label, int DB1, int num_class, int L, float **val_sim, int **ind_sim)
{
	...
	{				
		...
		{		
			...
			{
				if ( label[sup_loc[i][0]-1][x] * label[sup_loc[j][0]-1][x] == 1 )
				{
					active_L=1;
				}
				x++;
			...
			temp_sim[j]=(exp (-1*val[i][j])) * (active_L);   //if they do not share a label, then active_L=0 and their visual similarity=0
